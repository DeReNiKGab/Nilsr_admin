...
use App\Models\DataAndReport;
use Illuminate\Http\JsonResponse;

class DataAndReportsController extends Controller
{
    ...
    public function index($page): JsonResponse
    {
        ...
        $perPage = 12;
        $news = DataAndReport::whereNull('deleted_at')
            ->orderByDesc('id')
            ->offset(($page - 1) * $perPage)
            ->limit($perPage)
            ->get();

        $news_count = DataAndReport::whereNull('deleted_at')
    ...
    {
        $news = DataAndReport::where("id", $id)->whereNull('deleted_at')->first();
        if (!$news) {
