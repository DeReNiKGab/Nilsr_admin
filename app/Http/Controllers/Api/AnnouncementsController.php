...
class AnnouncementsController extends Controller
{
    ...
    {
        $perPage = 12;
        $news = Announcement::whereNull('deleted_at')
            ->orderByDesc('id')
            ->offset(($page - 1) * $perPage)
            ->limit($perPage)
            ->get();

        $news_count = Announcement::whereNull('deleted_at')
    ...
    public function AnnouncementsHeader(){
        $news = Announcement::whereNull('deleted_at')->orderByDesc('id')->take(5)->get();
        $data = [];
        foreach ($news as $key=>&$item) {
            $data[$key]['id'] = $item->id;
            $data[$key]['title'] = $item->title;
     ...
     * @return JsonResponse
     */
    public function AnnouncementsItem($id)
    {
        $news = Announcement::where("id", $id)->whereNull('deleted_at')->first();
        if (!$news) {
