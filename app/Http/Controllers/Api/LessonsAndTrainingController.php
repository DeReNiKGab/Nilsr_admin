<?php

namespace App\Http\Controllers\Api;

use App\Http\Controllers\Controller;
use App\Models\LessonsAndTraining;
use Illuminate\Http\JsonResponse;
use App\Helpers\ServerNameHelper;

class LessonsAndTrainingController extends Controller
{

    /**
     * @param $page
     * @return JsonResponse
     */
    public function index($page): JsonResponse
    {

        $perPage = 12;
        $news = LessonsAndTraining::whereNull('deleted_at')
            ->orderByDesc('id')
            ->offset(($page - 1) * $perPage)
            ->limit($perPage)
            ->get();

        $news_count = LessonsAndTraining::whereNull('deleted_at')
            ->selectRaw('count(id) as cnt')
            ->pluck('cnt')
            ->first();

        $data = [];

        foreach ($news as $key => $item) {
            $imgData = json_decode($item->img, true);
            $imgPath = '';

            if (!empty($imgData) && isset($imgData[0]['download_link'])) {
                $imgPath = ServerNameHelper::server_name() . "/storage/" . $imgData[0]['download_link'];
            }
            $data[$key]['id'] = $item->id;
            $data[$key]['title'] = $item->title;
            $data[$key]['description'] = $item->description;
            $data[$key]['img'] = $imgPath;
            $data[$key]['created_at'] = $item->created_at->format('Y-m-d');
            $data[$key]['count'] = $news_count;
        }
        return response()->json($data, 200);
    }

    public function DataAndReportsItem($id): JsonResponse
    {
        $news = LessonsAndTraining::where("id", $id)->whereNull('deleted_at')->first();
        if (!$news) {
            return response()->json(['error' => 'News item not found'], 404);
        }
        $data = [];
        $imgData = json_decode($news->img, true);
        $imgPath = '';
        if (!empty($imgData) && isset($imgData[0]['download_link'])) {
            $imgPath = ServerNameHelper::server_name() . "/storage/" . $imgData[0]['download_link'];
        }

        $data['id'] = $news->id;
        $data['title'] = $news->title;
        $data['description'] = $news->description;
        $data['img'] = $imgPath;
        $data['created_at'] = $news->created_at->format('Y-m-d');

        return response()->json($data, 200);
    }
}
